'''
    ...
    Input: nums = [5,5,5]
    Output: 15
    Explanation: The only possible polygon that can be made from nums has 3 sides: 5, 5, and 5. The perimeter
    is 5 + 5 + 5 = 15.
    Example 2:
...
class Solution(object):
    def largestPerimeter(self, nums):
        ...
        for index in reversed(range(len(sortedNums))):
            accumulatedSum = accumulatedSum + sortedNums[index]
            if (sortedNums[index] < (sumNums - accumulatedSum) ) and index >= 2:
                return (sumNums - accumulatedSum + sortedNums[index])
        return -1
